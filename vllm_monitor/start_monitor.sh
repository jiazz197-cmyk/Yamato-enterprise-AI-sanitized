...
echo -e "${GREEN}[success] Conda environment found${NC}"
...
echo -e "${GREEN}[success] Environment activated${NC}"
echo ""
...
if [ $MISSING -eq 1 ]; then
    echo -e "${RED}[error] Some packages are missing${NC}"
...
if command -v lsof &> /dev/null; then
    if lsof -Pi :9400 -sTCP:LISTEN -t >/dev/null 2>&1; then
        echo -e "${RED}[error] Port 9400 is already in use${NC}"
        echo "Kill existing process: sudo kill \$(lsof -t -i:9400)"
        exit 1
    fi
    echo -e "${GREEN}[success] Port 9400 is available${NC}"
fi

# Check NVIDIA GPU
if command -v nvidia-smi &> /dev/null; then
    echo -e "${GREEN}[success] NVIDIA driver detected${NC}"
